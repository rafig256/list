                        ...
                        @if($listings->count() >0)
                            @foreach($listings as $listing)
                                <tr>
                                    ...
                                    <td>
                                        <select class="form-control pending-select" data-id="{{$listing->id}}">
                                    ...
                                    <td class="text-center">
                                        <ul class="table-controls">
                                            ...
                                            <li><form action="{{route('admin.listing.destroy',$listing->id)}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button class="btn btn-link border-0 bg-transparent" data-toggle="tooltip" data-placement="top" title="Delete">
...
@push('scripts')
    <script>
        $(document).ready(function(){
            $('body').on('change','.pending-select',function (){
                let val = $(this).val();
                let id = $(this).data('id');
                alert(id);
                $.ajax({
                    method: 'POST',
                    url: '{{route('admin.listing.pending.update')}}',
                    data: {
                        _token:"{{csrf_token()}}",
                        id:id,
                        value:val,
