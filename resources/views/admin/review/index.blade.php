                    ...
                    <table class="table table-bordered">
                        ...
                        @foreach($reviews as $review)
                            <tr>
                                ...
                                <td>{{$review->user->name}}<br>{{$review->user->email}}</td>
                                <td>{{$review->listing->title}}</td>
                                <td>{{$review->text}}</td>
                                <td>
                                    @foreach($review->points as $point)
                                    {{$point->reviewCategory->name ?? "relation is destroyed"}}: {{$point->point}}<br>
                                    @endforeach
                                </td>
                                <td>{!! $review->status ? "<span class='badge badge-success'>Active</span>" : "<span class='badge badge-warning'>Pending</span>" !!}</td>
                                <td class="text-center">
                                    <ul class="table-controls">
                                        <li>
                                            <a href="{{$review->status ? route('admin.review.disable',$review->id) : route('admin.review.activate',$review->id)}}"  data-toggle="tooltip" data-placement="top" title="{{$review->status ? "disable" : "Activate" }}" >
                                                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                                                     viewBox="0 0 30.143 30.143" xml:space="preserve">
<g>
    <path style="fill:#{{$review->status ? 'FF0000' : '00FF00'}};" d="M20.034,2.357v3.824c3.482,1.798,5.869,5.427,5.869,9.619c0,5.98-4.848,10.83-10.828,10.83
    ...
    <path style="fill:#{{$review->status ? 'FF0000' : '00FF00'}};" d="M14.795,17.652c1.576,0,1.736-0.931,1.736-2.076V2.08c0-1.148-0.16-2.08-1.736-2.08
